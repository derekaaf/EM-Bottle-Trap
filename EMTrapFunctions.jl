...
using WGLMakie, Lux, Optimization, OptimizationEvolutionary, Optimisers, OptimizationMetaheuristics, Zygote, Base.Threads
using Statistics, LinearAlgebra, Random, StaticArrays, Bonito, ComponentArrays, Plots, ProgressMeter, Printf
...
function Loss(NN::Lux.Chain, st::NamedTuple, param::NamedTuple, inputs)
...
function Animate!(NN::Lux.Chain, sts::NamedTuple, params::NamedTuple, inputs)
    println("Animating...")
    track = nothing
    plt = nothing
 ...
 function Train!(NN, trainIt, params, sts, optimizer)
    ...
    for epoch in 1:trainIt
        ...
        (magnitude, loss, sts,), back = pullback(Fun, NN, sts, params, inputs)
